...
const EditIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3 ml-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
        <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path>
    </svg>
);

const ApiKeyStatus = ({ onClick }: { onClick: () => void }) => {
    const { apiKey, isDefault } = useApiKey();

    const truncateKey = (key: string) => {
        if (key.length <= 11) return key; // e.g. 7 + 4
    ...
    return (
        <button
            onClick={onClick}
            className="flex items-center text-neutral-400 hover:text-yellow-400 transition-colors duration-200 group"
            aria-label="Edit Gemini API Key"
        >
            <span>Gemini API Key:</span>
            <span className="font-mono ml-1.5 px-1.5 py-0.5 rounded-sm bg-neutral-700/50 text-yellow-300">
                {isDefault ? 'default' : truncateKey(apiKey!)}
            </span>
            <EditIcon />
        </button>
    );
...
const Footer = ({ onApiKeyClick }: { onApiKeyClick: () => void }) => {
    return (
        <footer className="fixed bottom-0 left-0 right-0 bg-black/50 backdrop-blur-sm p-3 z-50 text-neutral-300 text-sm border-t border-white/10">
            <div className="max-w-screen-xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-4 px-4 text-neutral-400">
                <div className="flex items-center gap-4">
                    <span className="text-neutral-500">©2025 Nam Vu</span>
                 ...
                 <div className="flex items-center gap-4">
                    <ApiKeyStatus onClick={onApiKeyClick} />
                    <LanguageToggle />
                </div>
            </div>
        </footer>
